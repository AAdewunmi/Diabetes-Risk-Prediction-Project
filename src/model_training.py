...
def build_pipeline(
    ...
    if model_name == "logreg":
        ...
        raise ValueError(
            f"Unsupported model: {model_name} (XGBoost available: {XGBOOST_AVAILABLE})"
        )
...
def train_and_tune(
    ...
    pipeline, param_dist = build_pipeline(
        model_name=model_name, random_state=random_state, use_smote=use_smote
    )
    ...
    if search:
        ...
        logging.info(
            "Hyperparameter search completed. Best params: %s", search.best_params_
        )
    ...
    try:
        ...
        logging.info(
            "Cross-val ROC AUC mean: %.4f (std %.4f)",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train models for Diabetes Risk Prediction"
    )
    ...
    parser.add_argument(
        "--smote",
        action="store_true",
        help="Apply SMOTE to training data (requires imbalanced-learn)",
    )
