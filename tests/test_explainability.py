...
def test_permutation_importance(tmp_path):
    ...
    assert csv is None or os.path.exists(csv)  # can be None if something fails, but usually exists
    if csv:
...
def test_shap_explain_small(tmp_path):
    ...
    from src.model_explainability_interpretability import shap_explain
    model_path, X, y = make_model_and_df(tmp_path)
    model = load_model_file(model_path)
